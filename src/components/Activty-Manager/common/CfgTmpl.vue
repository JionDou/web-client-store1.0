...
export default {
  ...
  watch: {
    ...
    // taskData(val) {
    //   if (this.taskData.type !== "new") {
    //     this.getById(this.taskData.id);
    //   }
    //   this.disabled = this.taskData.type === "view";
    // }
  },
  mounted() {
    this.getSchema();
  ...
  methods: {
    ...
    async getById(id) {
      ...
      if (type === "cfg") {
        const res = await editSchemaById(activity, id);
        if (res.code === 200 || res.code === 201) {
              ...
              x.properties.forEach(y => {
                ...
                if (y.cfg_type === "object") {
                  y.properties.forEach(z => {
                    ...
                    z.default =
                      taskData.pages[page_id][prop_id][prop_child_id][
                        "default"
                      ];
                  });
                }
        ...
        if (res.code === 200 || res.code === 201) {
          let taskData = res.data.content;
          this.ruleForm.name = res.data.meta_data.name; //配置模板类型
          this.ruleForm.id = res.data.meta_data.id; //配置模板id
    ...
    async onSure() {
      ...
      try {
        const res = await commitCfgTmplById(activity, id, data);
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: "提交成功！"
          ...
          console.log(this.$emit("closeDialog"));
        }
      } catch (error) {
        this.$message({
          type: "error",
