<template>
  <el-form
    ...
    label-width="120px"
  >
    <cfg-type v-show="isShowType"></cfg-type>
    <el-form-item
...
import CfgTmplType from "./CfgTmplType.vue";
import CfgType from "./CfgType.vue";
export default {
  name: "CfgSchmea",
  components: { vueJsonEditor, CfgTmplType, CfgType },
  props: { id: String },
  data() {
    return {
      isTarget: 0,
  ...
  methods: {
    ...
    async onSure() {
      ...
      this.beforeSubmit();
      let type = this.$route.mata.type;
      let activity = this.$route.meta.activity;
      let id = this.common[0].default;
      let data = this.ruleForm;
      try {
        const res = await commitSchemaById(activity, id, data);
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: "提交成功！"
