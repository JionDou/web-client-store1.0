  ...
  >
    ...
    >
      ...
      <el-select
        v-model="item.default"
        placeholder="请选择"
        v-if="item.cfg_type === 'single_select' && item.val_type === 'refer'"
        style="width:60%;"
      >
        <el-option
        ...
        <template v-if="item.cfg_type !== 'json'">
          ...
          >
            ...
            <el-select
              ...
              placeholder="请选择"
              v-if="
                item.cfg_type === 'single_select' && item.val_type === 'refer'
              "
              style="width:81%;"
            >
              <el-option
...
import CfgType from "./CfgType.vue";
import { getNumValModel } from "@/api/task";
...
export default {
  ...
  components: { vueJsonEditor, CfgType },
  data() {
    return {
      isSubmitVisble: true,
      tabPosition: "left",
  ...
  methods: {
    ...
    async getCfgTmplList() {
      this.loading = true;
      let activity = this.$route.meta.activity;
      const res = await getCfgTmplList(activity);
      if (res.code === 200 || res.code === 201) {
    ...
    async onSubmit() {
      ...
      try {
        ...
        const res = await commitSchemaById(activity, id, params);
        if (res.code === 200 || res.code === 201) {
          this.$message({
            type: "success",
            message: "提交成功！"
