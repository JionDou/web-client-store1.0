...
<script>
import { getNewTaskProcessAct, createNewTaskProcessActById } from "@/api/task";
export default {
  name: "AddTask",
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      this.pages.forEach(page => {
        ...
        console.log(page.items.properties);
        if (Array.isArray(page.items.properties)) {
          page.items.properties.forEach(children => {
      ...
      const res = await createNewTaskProcessActById(7, params, data);
      try {
        if (res.code === 200 || res.code === 201) {
          this.$message({
            type: "success",
