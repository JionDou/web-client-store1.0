...
import { createMobileTask, getEditTaskValData } from "@/api/task";
...
export default {
  ...
  methods: {
    ...
    async createMobileTask() {
      ...
      const res = await createMobileTask(params);
    ...
    //获取编辑任务数据
    async getEditTaskValData() {
      let data = "";
      const res = await getEditTaskValData(data, 2);
      console.log(res);
