...
<script>
// import { getModelFromData, createTaskById, getNumValModel } from "@/api/task";
import { getSchema, createSchemaById } from "@/api/m-Activity";
...
export default {
  ...
  methods: {
    ...
    //获取端游任务配置表单模板数据
    async getSchema() {
      const res = await getSchema("task");
    ...
    async onSubmit() {
      ...
      const res = await createSchemaById("task", id, params);
      try {
        if (res.code === 200 || res.code === 201) {
          this.$message({
            type: "success",
