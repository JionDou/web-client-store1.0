...
<script>
import { commitTaskById, getNumValModel } from "@/api/task";
import { getModelLibraryTableData } from "@/api/table.js";
import { getSchema, viewSchemaById } from "@/api/m-Activity";
...
export default {
  ...
  methods: {
    ...
    //获取编辑任务id
    async viewSchemaById(id) {
      console.log(id);
      const res = await viewSchemaById(id);
      try {
        if (res.code === 200 || res.code === 201) {
